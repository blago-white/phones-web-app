...
urlpatterns = [
    ...
    path("cards/", viewsets.PhoneCardViewSet.as_view({"get": "get_all"}), name="cards-list"),
    path("phone/<int:pk>/cards/", viewsets.PhoneCardViewSet.as_view(
        {"get": "get_related"}
    ), name="phone-cards-list"),
    path("phone/<int:pk>/card/", viewsets.PhoneCardViewSet.as_view(
        {"post": "create"}
    ), name="card"),
